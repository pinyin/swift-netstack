...
#if DEBUG
    // Contract: Every reply generated by Phases 7-9 is a well-formed, self-consistent
    // frame before it hits the wire. Each validator parses the full frame (Eth→IP→ICMP,
    // Eth→IP→UDP, or Eth→ARP) and verifies structural invariants plus checksums.
    //
    // ICMP replies: IPv4 checksum valid, ICMP type==0, ICMP checksum valid (recomputed
    //   from raw bytes — catches C2: stale 0xCC bytes in the checksum field).
    // DHCP replies: IPv4 checksum valid, protocol==UDP.
    // ARP replies:  operation==.reply (catches H3: request frames that leaked past
    //   processARPRequest's guard and got emitted as-is).
    //
    // The structural check (debugValidateReplies) then ensures every reply and forwarded
    // frame has a valid endpoint ID and ≥14 bytes — catching null-pointer-like issues
    // in frame construction before sendmsg(2) would silently fail or crash.
    for (_, pkt) in replies {
        // Dispatch reply validation based on EtherType
        if let eth = EthernetFrame.parse(from: pkt) {
            switch eth.etherType {
            case .ipv4:
                if let ip = IPv4Header.parse(from: eth.payload) {
                    switch ip.protocol {
                    case .icmp: debugValidateICMPReply(pkt)
                    case .udp:  debugValidateDHCPReply(pkt)
                    default:    break
                    }
                }
            case .arp:
                debugValidateARPReply(pkt)
            @unknown default:
                break
            }
        }
