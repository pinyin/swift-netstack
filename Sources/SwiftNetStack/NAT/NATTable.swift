...
public struct NATTable {
    ...
    private mutating func flushOneConnection(
        ...
        if conn.totalQueuedBytes > 0 {
            ...
            // Arm RTO timer if we sent new data and it's not already running
        ...
        // Window update: if the queue just drained and last advertised
        // window was 0 (or very small), tell the VM it can send again.
