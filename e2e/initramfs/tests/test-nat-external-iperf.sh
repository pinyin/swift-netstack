...
# 1. Single stream, short test
echo "  === 1. Single stream -t 1 ==="
/bin/iperf3 -c "$EXT_TARGET" -p "$EXT_IPERF_PORT" -t 1 -i 0 2>&1
RC1=$?
echo "  exit=$RC1"
sleep 1

# 2. Single stream, longer test
echo "  === 2. Single stream -t 3 ==="
/bin/iperf3 -c "$EXT_TARGET" -p "$EXT_IPERF_PORT" -t 3 -i 0 2>&1
RC2=$?
echo "  exit=$RC2"
sleep 1

# 3. 4 parallel streams
echo "  === 3. 4 parallel -t 3 ==="
/bin/iperf3 -c "$EXT_TARGET" -p "$EXT_IPERF_PORT" -t 3 -i 0 -P 4 2>&1
RC3=$?
echo "  exit=$RC3"
...
if [ $RC1 -eq 0 ] && [ $RC2 -eq 0 ] && [ $RC3 -eq 0 ]; then
    ...
    echo "  One or more iperf3 runs failed: rc1=$RC1 rc2=$RC2 rc3=$RC3"
